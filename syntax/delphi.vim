...
" THE FOLLOWING IS DONE BEFORE SET CASE IGNORE
" SO THAT MY REGULAR EXPRESSIONS ARE CASE SENSITIVE

" try matching all words that start with T as Pascal Objects
syn match pascalObject			'\sT[A-Z]\w\+[^;]'
" Add dev express components. Matching TcxAAA or TdxBBB
syn match pascalObject			'\sT[cd]x\w\+[^;]'
" Add report builder components
syn match pascalObject			'\sTpp\w\+[^;]'
" match all words that start with F as private member variables
" really they need to start with F followed by capital letter
" [\s\A^]F[A-Z]\w\+
syn match pascalDefine			'F[A-Z]\w\+'

"ODAC components     'TOra - any amount of not whitespace - not semicolon'
"syn match pascalObject			 'TOra\S\+[^;]'
"Dev Express components
"syn match pascalObject			'Tcx\S+[^;]'
"syn match pascalObject			'Tdx\S+[^;]'
